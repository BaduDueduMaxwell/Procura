...
def operations(_: AuthUser = Depends(admin_user)):
    with SessionLocal() as db:
        ...
        feedback_times = [item.time_to_first_feedback_ms for item in intake_rows if item.time_to_first_feedback_ms is not None]
        first_pass_rate = (
            sum(item.first_pass_complete for item in intake_rows) / len(intake_rows)
            if intake_rows else None
        )
        return OperationsSummary(request_count=len(rows), autonomous_recommendation_count=sum(r.decision == "recommended" for r in rows), human_review_count=sum(r.decision in ("review_required", "failed_safe") for r in rows), error_count=sum(r.decision == "failed_safe" for r in rows), p50_latency_ms=p50, p95_latency_ms=p95, token_usage=sum(measured_tokens) if measured_tokens else None, estimated_cost_usd=round(sum(measured_costs), 6) if measured_costs else None, evaluation_pass_rate=eval_rate, langfuse_status="Configured" if observability.langfuse_enabled else "Langfuse not configured", sentry_status="Configured" if observability.sentry_enabled else "Sentry not configured", recent_traces=traces, intake_count=len(intake_rows), intake_ready_count=sum(item.status == "ready" for item in intake_rows), intake_submitted_count=sum(item.status == "submitted" for item in intake_rows), intake_needs_correction_count=sum(item.status in {"needs_correction", "suggestion_available"} for item in intake_rows), intake_critical_review_count=sum(item.status == "critical_review_required" for item in intake_rows), intake_failed_safe_count=sum(item.status == "failed_safe" for item in intake_rows), intake_total_rows=sum((item.original_row_count or len(item.lines)) for item in intake_rows), intake_buyer_corrected_row_count=sum(bool(line.buyer_corrected_fields) or line.duplicate_resolution == "keep_both" for item in intake_rows for line in item.lines) + sum(len(item.removed_lines) for item in intake_rows), intake_first_pass_complete_rate=first_pass_rate, median_time_to_first_feedback_ms=statistics.median(feedback_times) if feedback_times else None, median_time_to_valid_submission_ms=statistics.median(intake_times) if intake_times else None)
