...
import { AlertTriangle, Check, ChevronLeft, ChevronRight, Download, FileSpreadsheet, LoaderCircle, RefreshCw, RotateCcw, Search, Send, Trash2, Upload, X } from "lucide-react";
...
export function BuyerIntake({ onNotice }: { onNotice?: (title: string, detail: string, tone?: "progress" | "success" | "attention" | "error") => void }) {
  ...
  return <main className="page-shell buyer-intake-shell validation-workspace">
    ...
    <section className="submission-summary"><div><p className="eyebrow">Final confirmation</p><h2>{current.status === "submitted" ? "Requirement submitted" : "Submission summary"}</h2><dl><div><dt>Original rows</dt><dd>{current.original_row_count ?? current.lines.length + (current.removed_lines?.length ?? 0)}</dd></div><div><dt>Active rows</dt><dd>{current.lines.length}</dd></div><div><dt>Ready rows</dt><dd>{count(current, "ready")}</dd></div><div><dt>Removed duplicates</dt><dd>{current.removed_lines?.length ?? 0}</dd></div><div><dt>Automatically normalized</dt><dd>{current.lines.filter(line => (line.normalized_fields?.length ?? 0) > 0).length}</dd></div><div><dt>Buyer-corrected rows</dt><dd>{current.lines.filter(line => (line.buyer_corrected_fields?.length ?? 0) > 0 || line.duplicate_resolution === "keep_both").length + (current.removed_lines?.length ?? 0)}</dd></div><div><dt>Accepted suggestions</dt><dd>{current.lines.filter(line => line.suggestion?.status === "accepted").length}</dd></div><div><dt>Remaining blockers</dt><dd>{current.lines.reduce((total, line) => total + line.findings.filter(item => item.severity === "blocker" || item.severity === "critical").length + (line.suggestion?.status === "pending" ? 1 : 0), 0)}</dd></div></dl>{Boolean(current.removed_lines?.length) && <div className="removed-row-list"><strong>Removed duplicate rows</strong>{current.removed_lines!.map(item => <div key={item.line.id}><span>{item.line.medicine_name} {item.line.strength} · source row {item.line.source_row}</span><button type="button" disabled={busy || current.status === "submitted"} onClick={() => run(() => api.resolveIntakeDuplicate(current.id, item.line.id, current.version, "restore"), "Restoring removed row")}><RotateCcw size={13}/>Restore</button></div>)}</div>}<p>No purchase order has been placed and no supplier has been contacted.</p><small>Validated {new Date(current.updated_at).toLocaleString()} · {current.policy_version} · Trace {current.trace_id}</small></div><button disabled={busy || current.status !== "ready"} onClick={() => run(() => api.submitIntake(current.id, current.version), "Recording submission")}><Check size={16}/>{current.status === "submitted" ? "Submitted" : "Confirm submission"}</button></section>
  </main>;
}
...
function IntakeRow({ line, intake, busy, run }: { line: IntakeLine; intake: ProcurementIntake; busy: boolean; run: (action: () => Promise<ProcurementIntake>, progress: string) => Promise<ProcurementIntake | undefined> }) {
  ...
  const duplicateFinding = line.findings.find(item => item.code === "possible_duplicate");
  const catalogueHelpCodes = new Set(["catalogue_variant_mismatch", "catalogue_match_required", "missing_destination", "missing_currency", "missing_max_lead_time_days", "missing_strength", "missing_dosage_form", "missing_pack_size"]);
  const showCatalogueOptions = line.suggestion?.status !== "pending" && line.findings.some(item => catalogueHelpCodes.has(item.code));
  ...
  return <>
    ...
    {open && <tr className="intake-row-detail"><td colSpan={6}><div>
      ...
      {showCatalogueOptions && Boolean(line.catalogue_options?.length) && <section className="catalogue-options" aria-label="Available catalogue options"><div className="catalogue-options-head"><div><strong>Available catalogue {line.catalogue_options!.length === 1 ? "variant" : "variants"}</strong><p>Choose a repository-backed variant. Your requested quantity and delivery requirement will not be changed.</p></div><span>{line.catalogue_options!.length} found</span></div><div className="catalogue-option-grid">{line.catalogue_options!.map(option => {
        const destinationCovered = !line.destination || option.destinations.includes(line.destination);
        const currencyCovered = !line.currency || option.currencies.includes(line.currency);
        const capacityCovered = !line.quantity || option.available_quantity_packs >= line.quantity;
        return <article key={option.source_record_id} className="catalogue-option"><div className="catalogue-option-title"><div><strong>{option.medicine_name} {option.strength}</strong><span>{option.dosage_form} · pack {option.pack_size}</span></div>{option.differences.length > 0 && <span className="option-change">{option.differences.join(" · ")}</span>}</div><dl><div><dt>Verified suppliers</dt><dd>{option.authorized_supplier_count}</dd></div><div><dt>Active quotations</dt><dd>{option.quotation_count}</dd></div><div><dt>Available capacity</dt><dd>{option.available_quantity_packs.toLocaleString()} packs</dd></div><div><dt>Fastest delivery</dt><dd>{option.minimum_lead_time_days} days</dd></div><div><dt>Destinations</dt><dd>{option.destinations.join(", ") || "None verified"}</dd></div><div><dt>Currency</dt><dd>{option.currencies.join(", ")}</dd></div><div><dt>Price range</dt><dd>{option.currencies.length === 1 ? `${option.currencies[0]} ` : ""}{option.unit_price_from.toFixed(2)}–{option.unit_price_to.toFixed(2)} / pack</dd></div><div><dt>Cold chain</dt><dd>{option.cold_chain_available ? "Available" : "Not listed"}</dd></div></dl>{(!destinationCovered || !currencyCovered || !capacityCovered) && <ul className="option-warnings">{!destinationCovered && <li>Requested destination {line.destination} is not covered. Available: {option.destinations.join(", ") || "none"}.</li>}{!currencyCovered && <li>Requested currency {line.currency} is not offered. Available: {option.currencies.join(", ")}.</li>}{!capacityCovered && <li>Current capacity is below the requested {line.quantity?.toLocaleString()} packs.</li>}</ul>}<div className="catalogue-option-actions">{option.destinations.map(destination => line.destination !== destination && <button type="button" key={destination} disabled={busy} onClick={() => run(() => api.patchIntakeLine(intake.id, line.id, intake.version, {destination}), `Using ${destination} as destination`)}>Use {destination}</button>)}{option.currencies.map(currency => line.currency !== currency && <button type="button" key={currency} disabled={busy} onClick={() => run(() => api.patchIntakeLine(intake.id, line.id, intake.version, {currency}), `Using ${currency} as currency`)}>Use {currency}</button>)}<button type="button" className="use-variant" disabled={busy} onClick={() => run(() => api.selectIntakeVariant(intake.id, line.id, intake.version, option.source_record_id), "Applying selected catalogue variant")}><Check size={14}/>Use this variant</button></div><small>{line.quantity ? `${line.quantity.toLocaleString()} ${line.unit ?? "packs"} will remain unchanged.` : "Enter the required quantity before submission."}</small></article>;
      })}</div></section>}
      {showCatalogueOptions && !line.catalogue_options?.length && <section className="catalogue-empty" role="status"><strong>No supported catalogue variant found</strong><p>Check the medicine name or use the fields below. Procura will not invent a medicine match.</p></section>}
      {duplicateFinding && <section className="duplicate-resolution" role="alert"><div><strong>Is this row intentional?</strong><p>This medicine requirement matches another row. Remove this row if it was added twice, or keep both when they are separate requirements.</p></div><div><button type="button" className="remove-duplicate" disabled={busy} onClick={() => run(() => api.resolveIntakeDuplicate(intake.id, line.id, intake.version, "remove"), "Removing duplicate row")}><Trash2 size={14}/>Remove this row</button><button type="button" disabled={busy} onClick={() => run(() => api.resolveIntakeDuplicate(intake.id, line.id, intake.version, "keep_both"), "Confirming separate requirements")}><Check size={14}/>Keep both rows</button></div></section>}
      {line.findings.some(item => item.code !== "possible_duplicate") && <ul className="finding-list">{line.findings.filter(item => item.code !== "possible_duplicate").map(item => <li key={item.code} className={`finding-${item.severity}`}><AlertTriangle size={14}/><span><strong>{item.message}</strong><small>{item.suggested_action} · Evidence: {item.evidence_source}</small></span></li>)}</ul>}
