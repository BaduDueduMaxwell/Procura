...
function IntakeRow({ line, intake, busy, run }: { line: IntakeLine; intake: ProcurementIntake; busy: boolean; run: (action: () => Promise<ProcurementIntake>, progress: string) => Promise<ProcurementIntake | undefined> }) {
  ...
  return <>
    ...
    {open && <tr className="intake-row-detail"><td colSpan={6}><div>
      {line.suggestion?.status === "pending" && <section className="catalogue-suggestion"><div><strong>Catalogue suggestion</strong><p>“{line.suggestion.original_value}” may be <b>{line.suggestion.suggested_value}</b>. {line.suggestion.match_reason}. Source: {line.suggestion.source_record_id}.</p></div><div><button disabled={busy} onClick={() => run(() => api.decideIntakeSuggestion(intake.id, line.id, intake.version, "reject"), "Rejecting suggestion")}><X size={14}/>Reject</button><button disabled={busy} onClick={() => run(() => api.decideIntakeSuggestion(intake.id, line.id, intake.version, "accept"), "Applying confirmed suggestion")}><Check size={14}/>Accept</button></div></section>}
