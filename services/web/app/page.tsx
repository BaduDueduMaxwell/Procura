...
import type { AdminOverview, AdminUserPage, AgentResponse, AuthUser, DashboardDecision, MedicineCatalogItem, Notification, Operations, ProcurementLifecycle, ReviewBrief, ReviewCase, SupplierDashboard as SupplierDashboardData, SupplierQuoteDraft, SupplierRequestAssignment, SupplierSubmission } from "@/lib/types";
...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    {screen === "dashboard" && ["buyer", "admin"].includes(user.role) && <CustomerDashboard onStart={() => { navigate("intake"); notify("New requirement ready", "Enter one requirement or upload a procurement list."); }} onOpenDecision={traceId => openDecision(traceId)} onOpenIntake={intakeId => navigate("intake", false, `/intake/${encodeURIComponent(intakeId)}`)} />}
    ...
    {screen === "chat" && ["buyer", "admin"].includes(user.role) && <main className="workspace">
      ...
      <section className="conversation" aria-label="Procurement conversation">
        <div className="conversation-head"><div><p className="eyebrow">Supplier comparison</p><h1>Compare verified quotations</h1><p>Use a complete requirement to check supplier eligibility, exclusions, and the recommended next step.</p></div><div className="head-actions"><button className="quiet-button tablet-evidence" onClick={() => setDrawer(true)} aria-label="Open decision evidence"><PanelRight size={16} />Evidence</button><button className="quiet-button desktop-only" onClick={startNew}><Plus size={16} />New comparison</button></div></div>
...
function Nav({ screen, setScreen, onNew, onGuide, user, onLogout }: { screen: Screen; setScreen: (s: Screen) => void; onNew: () => void; onGuide: () => void; user: AuthUser; onLogout: () => void }) {
  const items: [Screen, string, typeof MessageSquareText][] = [["dashboard", "Dashboard", LayoutDashboard], ["intake", "Requirements", FileSearch], ["chat", "Supplier comparison", MessageSquareText], ["reviews", "Request reviews", ClipboardCheck], ["supplierReviews", "Supplier approvals", Building2], ["operations", "Operations", Activity], ["admin", "Administration", UsersRound]];
...
function tourSteps(role: AuthUser["role"]): TourStep[] {
  ...
  const steps: TourStep[] = [
    ...
    { title: "Start from the dashboard", description: "See requirements needing correction, ready submissions, completed submissions, and supplier comparison outcomes in one place.", screen: "dashboard", target: "dashboard" },
    { title: "Prepare the requirement", description: "Select Requirements to enter one need or upload a procurement list, then resolve missing or mismatched fields before submission.", screen: "intake", target: "nav-intake" },
    { title: "Compare supplier quotations", description: "Select Supplier comparison after the requirement is complete to evaluate eligibility, exclusions, and the recommended next step.", screen: "chat", target: "nav-chat" },
...
function Reviews() {
  ...
  const groupedReasons = selected ? summarizeReviewReasons(selected.reasons, selected.request.max_lead_time_days, selected.quotes.length) : [];
  const noEligibleQuote = Boolean(selected?.quotes.length) && selected?.quotes.every(quote => !quote.eligible);
  const briefContext = brief?.evidence_points.filter(point => /^(Request|Supplier quotations checked|Current recommendation):/.test(point)) ?? [];
...
function OperationsScreen() {
  ...
  const duration = (value?: number) => value != null ? `${value < 1000 ? Math.round(value) + " ms" : (value / 1000).toFixed(1) + " s"} median` : "Not enough data";
  const metrics = [["Intakes", data.intake_count ?? 0], ["Submitted", data.intake_submitted_count ?? 0], ["Needs correction", data.intake_needs_correction_count ?? 0], ["Critical review", data.intake_critical_review_count ?? 0], ["First-pass ready", data.intake_first_pass_complete_rate != null ? `${Math.round(data.intake_first_pass_complete_rate * 100)}%` : "Not enough data"], ["Time to valid", duration(data.median_time_to_valid_submission_ms)]];
  const health = [["Rows processed", data.intake_total_rows ?? 0], ["Buyer-corrected rows", data.intake_buyer_corrected_row_count ?? 0], ["First feedback", duration(data.median_time_to_first_feedback_ms)], ["Failed safely", data.intake_failed_safe_count ?? 0]];
  return <main className="page-shell" data-tour="operations"><header className="page-head"><div><p className="eyebrow">Buyer intake performance</p><h1>Operations</h1><p>Track how quickly procurement lists become complete submissions and where buyers need help.</p></div></header><div className="metric-grid operations-metrics">{metrics.map(([label, value]) => <div className="metric" key={label}><span>{label}</span><strong>{value}</strong></div>)}</div><div className="ops-grid"><section className="data-card"><div className="card-title"><h2>Recent agent decisions</h2><span>{data.recent_traces.length} records</span></div>{data.recent_traces.length ? <div className="trace-list">{data.recent_traces.map(t => <button type="button" className={selectedTrace?.trace_id === t.trace_id ? "selected" : ""} key={t.trace_id} onClick={() => { setSelectedTrace(t); window.history.pushState({}, "", `/operations/traces/${t.trace_id}`); }} aria-label={`Open ${medicineLabel(t.medicine_name, t.strength)} trace`}><span><strong>{medicineLabel(t.medicine_name, t.strength)}</strong><small>{t.decision.replaceAll("_", " ")} · {t.trace_id.slice(0, 8)}</small></span><strong>{t.latency_ms} ms</strong><ArrowRight size={14}/></button>)}</div> : <p className="muted-block">No agent decisions yet. Complete a request to create the first trace.</p>}{selectedTrace && <div className="trace-detail" aria-live="polite"><div className="card-title"><h3>{medicineLabel(selectedTrace.medicine_name, selectedTrace.strength, "Trace evidence")}</h3><button type="button" onClick={() => { setSelectedTrace(undefined); window.history.pushState({}, "", "/operations"); }} aria-label="Close trace evidence"><X size={14}/></button></div><dl><div><dt>Trace ID</dt><dd><code>{selectedTrace.trace_id}</code></dd></div><div><dt>Provider</dt><dd>{selectedTrace.provider} · {selectedTrace.model}</dd></div><div><dt>Decision</dt><dd>{selectedTrace.decision.replaceAll("_", " ")}</dd></div><div><dt>Policy</dt><dd>{selectedTrace.policy_version}</dd></div><div><dt>Tokens</dt><dd>{selectedTrace.token_input ?? "Not available"} in · {selectedTrace.token_output ?? "Not available"} out</dd></div><div><dt>Tool calls</dt><dd>{selectedTrace.tool_sequence.length}</dd></div></dl></div>}</section><section className="data-card"><div className="card-title"><h2>Workflow health</h2></div><dl className="workflow-health">{health.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><h3 className="system-status-title">Monitoring</h3><div className="integration"><span className={data.langfuse_status === "Configured" ? "good-dot" : "neutral-dot"} /><div><strong>Trace export</strong><p>{data.langfuse_status === "Configured" ? "Connected" : "Using local records"}</p></div></div><div className="integration"><span className={data.sentry_status === "Configured" ? "good-dot" : "neutral-dot"} /><div><strong>Error monitoring</strong><p>{data.sentry_status === "Configured" ? "Connected" : "Using local logs"}</p></div></div><p className="fine-print">Procura preserves local workflow records when external monitoring is not connected.</p></section></div></main>;
}
