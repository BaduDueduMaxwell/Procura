...
function Reviews() {
  ...
  const chooseCase = (item: ReviewCase | undefined) => { setSelected(item); setBrief(undefined); if (!item) return; setBriefLoading(true); api.reviewBrief(item.id).then(setBrief).catch(() => setBrief(undefined)).finally(() => setBriefLoading(false)); };
  const load = () => api.reviews().then(items => { setCases(items); setSelected(current => { const next = items.find(x => x.id === current?.id) || items[0]; if (next) { setBriefLoading(true); api.reviewBrief(next.id).then(setBrief).catch(() => setBrief(undefined)).finally(() => setBriefLoading(false)); } return next; }); }).catch(() => setError(true));
  useEffect(() => { load(); }, []);
