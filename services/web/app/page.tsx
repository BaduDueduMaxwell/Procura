...
import type { AgentResponse, AuthUser, CustomerDashboard as CustomerDashboardData, Operations, ReviewCase, SupplierDashboard as SupplierDashboardData } from "@/lib/types";
import { DecisionCards } from "@/components/DecisionCards";
import { EvidencePanel } from "@/components/EvidencePanel";
import { StatusBadge } from "@/components/StatusBadge";

type Screen = "dashboard" | "chat" | "supplier" | "reviews" | "operations";
...
const examples = [
  "5,000 packs of paracetamol 500 mg tablets, pack size 100, delivered to Accra within 18 days in USD.",
...
export default function Home() {
  ...
  useEffect(() => { api.me().then(account => { setUser(account); setScreen(account.role === "supplier" ? "supplier" : "dashboard"); }).catch(() => undefined).finally(() => setAuthReady(true)); }, []);
  ...
  if (!user) return <Landing onAuthenticated={account => { setUser(account); setScreen(account.role === "supplier" ? "supplier" : "dashboard"); }} />;
  return <div className="app-shell">
    <Nav screen={screen} setScreen={setScreen} onNew={() => { setScreen("chat"); startNew(); }} onGuide={() => setTourStep(0)} user={user} onLogout={() => api.logout().finally(() => { setUser(undefined); setConversationId(undefined); })} />
    {screen === "dashboard" && user.role !== "supplier" && <CustomerDashboard onStart={() => { setScreen("chat"); startNew(); }} />}
    ...
    {tourStep !== null && <ProductTour user={user} stepIndex={tourStep} onStep={setTourStep} onNavigate={setScreen} onClose={closeTour} />}
  </div>;
}
...
function Nav({ screen, setScreen, onNew, onGuide, user, onLogout }: { screen: Screen; setScreen: (s: Screen) => void; onNew: () => void; onGuide: () => void; user: AuthUser; onLogout: () => void }) {
  const items: [Screen, string, typeof MessageSquareText][] = [["dashboard", "Dashboard", LayoutDashboard], ["chat", "Workspace", MessageSquareText], ["reviews", "Staff review", ClipboardCheck], ["operations", "Operations", Activity]];
  const visible = user.role === "supplier" ? [["supplier", "Supplier portal", Building2]] as [Screen, string, typeof MessageSquareText][] : user.role === "buyer" ? items.filter(([id]) => !["reviews", "operations"].includes(id)) : items;
...
function Landing({ onAuthenticated }: { onAuthenticated: (user: AuthUser) => void }) {
  ...
  async function authenticate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const account = mode === "login" ? await api.login(email, password) : await api.signup({ email, password, display_name: String(form.get("name") || ""), organization: String(form.get("organization") || "") });
      onAuthenticated(account);
    } catch (error) { setFailure(error instanceof Error ? error.message : "Authentication failed"); }
    finally { setBusy(false); }
  }
  return <main className="landing"><nav className="landing-nav"><Brand /><div><a href="#about">How it works</a><a href="#workflow">Workflow</a><button onClick={() => setMode("login")}>Sign in</button></div></nav><section className="hero"><div className="hero-copy"><p className="eyebrow">Medicine procurement, brought into focus</p><h1>Move every request from need to decision.</h1><p>Procura gives buying teams one place to capture medicine requirements, compare qualified supplier quotations, resolve exceptions, and record approvals.</p><div className="trust-row"><span><Check size={15}/>Clear requirements</span><span><Check size={15}/>Comparable quotations</span><span><Check size={15}/>Accountable approvals</span></div></div><section className="auth-card" aria-labelledby="auth-title"><div className="auth-tabs"><button className={mode === "signup" ? "active" : ""} onClick={() => setMode("signup")}>Create account</button><button className={mode === "login" ? "active" : ""} onClick={() => setMode("login")}>Sign in</button></div><h2 id="auth-title">{mode === "signup" ? "Start your workspace" : "Welcome back"}</h2><p>{mode === "signup" ? "Create your procurement workspace and bring every request into one clear process." : "Continue to your procurement workspace."}</p><form onSubmit={authenticate}>{mode === "signup" && <><label>Full name<input name="name" minLength={2} maxLength={80} autoComplete="name" required /></label><label>Organization<input name="organization" minLength={2} maxLength={120} autoComplete="organization" required /></label></>}<label>Email address<input name="email" type="email" maxLength={320} autoComplete="email" required /></label><label>Password<input name="password" type="password" minLength={12} maxLength={128} autoComplete={mode === "signup" ? "new-password" : "current-password"} required /></label>{mode === "signup" && <small>Use 12 or more characters with upper and lowercase letters, a number, and a symbol.</small>}{failure && <div className="auth-error" role="alert">{failure}</div>}<button className="auth-submit" disabled={busy}>{busy ? "Please wait…" : mode === "signup" ? "Create workspace" : "Sign in"}<ArrowRight size={17}/></button></form></section></section><section className="feature-band" id="about"><article data-reveal><span>01</span><h2>Capture the full requirement</h2><p>Turn a conversation into a complete procurement brief and clarify only what is missing.</p></article><article data-reveal><span>02</span><h2>Compare qualified suppliers</h2><p>See which quotations meet the request, why others were excluded, and which option leads.</p></article><article data-reveal><span>03</span><h2>Resolve exceptions together</h2><p>Give reviewers the evidence, reasons, and recommended action needed to keep work moving.</p></article></section><section className="security-strip" id="workflow" data-reveal><Shield size={26}/><div><h2>A clear path from request to recommendation</h2><p>Procura checks supplier fit, explains every exclusion, ranks eligible quotations, and sends exceptions to the right reviewer so teams can move faster without losing control.</p></div></section><footer>Procura · Procurement decisions, clearly managed</footer></main>;
}
...
function SupplierDashboard() {
  const [data, setData] = useState<SupplierDashboardData>(); const [failed, setFailed] = useState(false);
  useEffect(() => { api.supplierDashboard().then(setData).catch(() => setFailed(true)); }, []);
  ...
  return <main className="page-shell" data-tour="supplier-overview"><header className="page-head"><div><p className="eyebrow">Supplier workspace</p><h1>{supplier.display_name}</h1><p>Manage your authorization profile, service capabilities, and active quotations.</p></div><StatusBadge status={data.compliance_state === "authorized" ? "eligible" : "review"} /></header><div className="metric-grid supplier-metrics"><div className="metric"><span>Active quotations</span><strong>{data.quote_count}</strong></div><div className="metric"><span>Destinations</span><strong>{data.eligible_destination_count}</strong></div><div className="metric"><span>Reliability</span><strong>{Math.round(supplier.reliability_score * 100)}%</strong></div><div className="metric"><span>Cold chain</span><strong>{supplier.capability.cold_chain ? "Capable" : "Not listed"}</strong></div></div><div className="dashboard-grid"><section className="data-card" data-tour="supplier-compliance"><div className="card-title"><ClipboardCheck size={17}/><h2>Compliance evidence</h2></div><dl className="supplier-evidence"><dt>Authorization</dt><dd>{supplier.authorization.status}</dd><dt>Expiry</dt><dd>{supplier.authorization.expiry_date || "Not recorded"}</dd><dt>Supported markets</dt><dd>{supplier.capability.destinations.join(", ")}</dd></dl></section><section className="data-card" data-tour="supplier-quotes"><div className="card-title"><PackageCheck size={17}/><h2>Submitted quotations</h2></div>{supplier.quotes.map(quote => <div className="supplier-quote" key={quote.id}><div><strong>{quote.line.medicine_name} {quote.line.strength}</strong><span>{quote.line.dosage_form} · pack {quote.line.pack_size} · {quote.lead_time_days} days</span></div><strong>{quote.currency} {quote.line.unit_price.toFixed(2)} / pack</strong></div>)}</section></div><div className="supplier-boundary"><Shield size={18}/><p>Profile changes are subject to verification. Submitted quotations do not create or confirm an order.</p></div></main>;
}
