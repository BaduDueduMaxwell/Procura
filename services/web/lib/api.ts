...
export const api = {
  ...
  selectIntakeVariant: (intakeId: string, lineId: string, version: number, sourceRecordId: string) => request<ProcurementIntake>(`/api/intakes/${intakeId}/lines/${lineId}/variant`, { method: "POST", body: JSON.stringify({ source_record_id: sourceRecordId, version, idempotency_key: crypto.randomUUID() }) }),
  resolveIntakeDuplicate: (intakeId: string, lineId: string, version: number, action: "remove" | "keep_both" | "restore") => request<ProcurementIntake>(`/api/intakes/${intakeId}/lines/${lineId}/duplicate`, { method: "POST", body: JSON.stringify({ action, version, idempotency_key: crypto.randomUUID() }) }),
